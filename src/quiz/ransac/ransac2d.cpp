...
/** 
 * \brief Ransac 3D, extended as per instructor's implementation \b Ransac2
 * \param[in] cloud a PCD
 * \param[in] maxIterations max iterations before stopping
 * \param[in] distanceTol tolerated distance between points
 */
...
int main ()
{
	...
	// std::unordered_set<int> inliers = Ransac2(cloud, 100, .2);
	std::unordered_set<int> inliers = Ransac3D(cloud, 100, .25);
